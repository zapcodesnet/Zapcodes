...
export default function Build() {
  ...
  const coinDataLoaded = coinData !== null;

  // canGenerate: check daily cap + char limit on frontend; backend enforces BL balance
  // When coinData hasn't loaded yet, allow clicking (backend will validate everything)
  const canGenerate = coinDataLoaded
    ? (gensUsed < genCap && charsUsed <= (maxChars === Infinity ? 999999 : maxChars))
    : true;

  // ══════════ GENERATE ══════════
  const handleGenerate = async () => {
    if (!prompt.trim()) return alert('Enter a description');
    if (coinDataLoaded && !canGenerate) return alert(gensUsed >= genCap ? 'Daily generation limit reached' : 'Message too long for your plan');
