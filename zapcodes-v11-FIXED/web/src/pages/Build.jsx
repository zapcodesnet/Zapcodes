...
// Backend sends 999999999 for unlimited values (Infinity can't be serialized to JSON)
// This sentinel is ONLY for tier config (gen caps, char limits) — NOT for BL coin balance
const UNLIMITED = 999999999;
function isUnlimited(n) { return n >= UNLIMITED; }

// Format BL coin amounts — always show real numbers, never ∞
function formatBL(n) {
  if (n == null) return '0';
...
export default function Build() {
  ...
  // canGenerate: check daily gen cap + char limit on frontend.
  // Backend enforces BL coin balance — do NOT check balance here.
  // When coinData hasn't loaded yet, allow the button (backend validates everything).
  ...
  const handleGenerate = async () => {
    ...
    if (dataReady && !isUnlimited(maxChars) && charsUsed > maxChars) return alert(`Message too long. Your plan allows ${maxChars.toLocaleString()} characters.`);
              ...
              {tc.dailyFixCap === 0 ? (
                ...
                <>
                  <p style={{ color: 'var(--text-secondary)', fontSize: 13, marginBottom: 16 }}>
                    Upload files and describe the issue. ({du.codeFixes || 0}/{isUnlimited(tc.dailyFixCap) ? '∞' : tc.dailyFixCap} fixes used today)
                  </p>
