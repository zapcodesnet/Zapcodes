...
export default function Build() {
  ...
  const canGenerate = dataReady ? (gensUsed < genCap && charsUsed <= (maxChars === Infinity ? 999999 : maxChars)) : true;

  // ══════════ GENERATE ══════════
  const handleGenerate = async () => {
    if (!prompt.trim()) return alert('Enter a description');
    if (dataReady && !canGenerate) return alert(gensUsed >= genCap ? 'Daily generation limit reached' : 'Message too long for your plan');
