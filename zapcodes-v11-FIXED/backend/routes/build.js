...
// ══════════ SYSTEM PROMPTS ══════════
const GEN_PROMPT = `You are ZapCodes AI, an expert full-stack web developer. Generate COMPLETE, production-quality, WORKING websites that render beautifully in a browser.

CRITICAL RULES:
1. Output COMPLETE file contents — NEVER use placeholders like "// rest of code here" or "..." or "/* more styles */". Every file must be 100% complete and functional.
2. The index.html file MUST be a fully self-contained HTML document with ALL CSS in a <style> tag inside <head> and ALL JavaScript in a <script> tag before </body>. This is REQUIRED because the output must work as a single HTML file in an iframe preview.
3. Use modern CSS: CSS variables, flexbox/grid, animations, gradients, shadows, smooth transitions, hover effects.
4. Professional design: dark themes with accent colors, card-based layouts, proper spacing, typography hierarchy, border-radius on elements, box-shadows.
5. Mobile-first responsive design using media queries.
6. Include ALL sections and features the user describes — never skip or abbreviate.
7. Format each file as: \`\`\`filepath:filename.ext\n(content)\n\`\`\`
8. The index.html file MUST include: <!DOCTYPE html>, <html lang="en">, <head> with meta viewport, <title>, complete <style> block, complete <body>, and complete <script> block.
9. Use semantic HTML5 elements (header, nav, main, section, footer, article).
10. All interactive elements MUST have working JavaScript event handlers.
11. Include smooth scroll behavior, fade-in animations, and hover transitions.
12. Use a cohesive color palette with CSS custom properties (--primary, --secondary, --bg, --text, etc).
13. Typography: Use system font stack or Google Fonts via CDN. Set proper font sizes, weights, and line heights.
14. Forms must have working validation and visual feedback.
15. Navigation must work with smooth scroll to sections.
16. Images: Use placeholder services like https://picsum.photos/800/400 or SVG placeholders.

IMPORTANT: The PRIMARY output must be a SINGLE index.html file with everything inlined (CSS in <style>, JS in <script>). You may also output separate style.css and script.js files for download purposes, but the index.html MUST be self-contained and fully functional on its own.`;

const FIX_PROMPT = `You are ZapCodes AI, an expert code debugger and web developer. Fix the provided code to make it fully functional and visually complete.
...
7. Ensure all interactive elements work (buttons, forms, navigation, modals, etc).
8. Fix any broken layouts, missing styles, or non-functional JavaScript.
9. If CSS is in a separate file, merge it into the index.html <style> block.
10. If JS is in a separate file, merge it into the index.html <script> block.`;
...
function generatePreviewHTML(files) {
  // First, try to find index.html - it should be self-contained
  ...
  if (!html) {
    // No HTML file — wrap any content we have into a basic page
    const cssContent = css ? css.content : '';
    const jsContent = js ? js.content : '';
    return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Preview</title><style>${cssContent}</style></head><body><h1>Preview</h1><p>No index.html file was generated.</p><script>${jsContent}</script></body></html>`;
  }
  ...
  // Inject CSS if it exists and isn't already inlined
  if (css && css.content.trim()) {
    const cssSnippet = css.content.trim().substring(0, 60);
    if (!content.includes(cssSnippet)) {
      // Remove any <link> to external stylesheet and inject inline
      ...
      if (content.includes('</head>')) {
        ...
        content = content.replace(/<body/i, `<style>\n${css.content}\n</style>\n<body`);
      }
    }
  }

  // Inject JS if it exists and isn't already inlined
  if (js && js.content.trim()) {
    const jsSnippet = js.content.trim().substring(0, 60);
    if (!content.includes(jsSnippet)) {
      // Remove any <script src="script.js"> and inject inline
...
IMPORTANT: The index.html file MUST be completely self-contained with ALL CSS inside a <style> tag in <head> and ALL JavaScript inside a <script> tag before </body>. Do NOT use external file references like <link href="style.css"> or <script src="script.js">. Everything must be inlined in the single HTML file.`;
...
router.post('/deploy', auth, async (req, res) => {
  try {
    ...
    // Inject badge (unless Gold/Diamond with badge removal privilege, or site badge already removed)
    let deployFiles = files;
    const canRemoveBadge = config.canRemoveBadge;
    const hasBadge = !canRemoveBadge || (existingSite && existingSite.hasBadge !== false);
    const shouldInjectBadge = !canRemoveBadge; // Free, Bronze, Silver always get badge
    ...
    if (existingSite) {
      ...
      user.deployedSites.push({
        subdomain: sub, title: title || sub, hasBadge: shouldInjectBadge,
