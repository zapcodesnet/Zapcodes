...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    api.get('/admin/check-access', { headers }).then(({ data }) => {
  ...
  const loadAdminData = () => {
    const headers = adminSession ? { 'x-admin-session': adminSession } : {};
    api.get('/admin/me', { headers }).then(({ data }) => {
      setAdminUser(data);
      setLoading(false);
    }).catch(() => {
      setAccessState('needs_verification');
  ...
  const sendCode = async () => {
    ...
    setVerifyMsg('');
    try {
      const { data } = await api.post('/admin/send-code');
      setVerifyMsg(data.message || 'Code sent!');
      if (data.devCode) setVerifyMsg(`Code sent! (Dev mode: ${data.devCode})`);
    } catch (err) {
      setVerifyMsg(err.response?.data?.error || 'Failed to send code');
    }
  ...
  const submitCode = async () => {
    ...
    setVerifyMsg('');
    try {
      const { data } = await api.post('/admin/verify-code', { code: verifyCode });
      ...
      setVerifyMsg(err.response?.data?.error || 'Invalid code');
    }
    setVerifyLoading(false);
  };
    ...
    return (
      <div style={s.verifyPage}>
        <div style={s.verifyCard}>
          ...
          <h2 style={{ marginTop: 12, fontSize: '1.3rem' }}>Admin Verification</h2>
          <p style={{ color: '#888', fontSize: '0.9rem', marginTop: 8 }}>Please log in to your ZapCodes account first.</p>
          <button onClick={() => navigate('/login')} style={s.verifyBtn}>Log In →</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={s.verifyPage}>
        <div style={s.verifyCard}>
          <span style={{ fontSize: '2.5rem' }}>🔐</span>
          <h2 style={{ marginTop: 12, fontSize: '1.3rem' }}>Admin Verification</h2>
          <p style={{ color: '#888', fontSize: '0.9rem', marginTop: 8 }}>Enter the verification code sent to your admin email.</p>
          <button onClick={sendCode} style={s.verifyBtn} disabled={verifyLoading}>{verifyLoading ? 'Sending...' : 'Send Verification Code'}</button>
          {verifyMsg && <p style={{ color: '#ffaa00', fontSize: '0.8rem', marginTop: 12 }}>{verifyMsg}</p>}
          <div style={{ display: 'flex', gap: 8, marginTop: 16, width: '100%' }}>
            <input value={verifyCode} onChange={e => setVerifyCode(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="6-digit code" style={s.verifyInput} maxLength={6} />
            <button onClick={submitCode} style={{ ...s.verifyBtn, flex: 'none', padding: '10px 24px' }} disabled={verifyLoading || verifyCode.length < 6}>Verify</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div style={s.verifyPage}>
        <div style={s.verifyCard}>
          <span style={{ fontSize: '2.5rem' }}>🔐</span>
          <h2 style={{ marginTop: 12, fontSize: '1.3rem' }}>Admin Verification</h2>
          <p style={{ color: '#888', fontSize: '0.9rem', marginTop: 8 }}>For security, enter the verification code sent to your admin email.</p>
          <button onClick={sendCode} style={s.verifyBtn} disabled={verifyLoading}>{verifyLoading ? 'Sending...' : 'Send Verification Code'}</button>
          {verifyMsg && <p style={{ color: verifyMsg.includes('Dev') ? '#00e5a0' : '#ffaa00', fontSize: '0.8rem', marginTop: 12 }}>{verifyMsg}</p>}
          <div style={{ display: 'flex', gap: 8, marginTop: 16, width: '100%' }}>
            <input value={verifyCode} onChange={e => setVerifyCode(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="6-digit code" style={s.verifyInput} maxLength={6} onKeyDown={e => e.key === 'Enter' && submitCode()} autoFocus />
            <button onClick={submitCode} style={{ ...s.verifyBtn, flex: 'none', padding: '10px 24px' }} disabled={verifyLoading || verifyCode.length < 6}>Verify</button>
          </div>
        </div>
      </div>
...
const s = {
  ...
  verifyPage: { display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', background: '#06060b', color: '#e8e8f0' },
  verifyCard: { background: '#11111b', border: '1px solid #1a1a2a', borderRadius: 16, padding: '48px 40px', maxWidth: 400, width: '100%', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' },
  verifyBtn: { padding: '12px 28px', borderRadius: 10, border: 'none', background: '#00e5a0', color: '#06060b', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer', marginTop: 16, width: '100%' },
  verifyInput: { background: '#0a0a14', border: '1px solid #2a2a3a', borderRadius: 10, padding: '12px 16px', color: '#e8e8f0', fontSize: '1.2rem', textAlign: 'center', letterSpacing: 8, flex: 1, fontFamily: 'var(--font-mono)' },
};
