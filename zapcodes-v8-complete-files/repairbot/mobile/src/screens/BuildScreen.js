...
export default function BuildScreen() {
  ...
  const sendAiChat = async () => {
    ...
    try {
      ...
      const { data } = await api.post('/files/analyze', { files: uploadedFiles, prompt: msg, mode });
      ...
      } else if (data.generatedFiles?.length) {
      ...
      {uploadedFiles.length === 0 ? (
        <View>
          <Text style={{ color: '#888', fontSize: 14, marginBottom: 16, lineHeight: 22 }}>
            Upload your code files — Moltbot will analyze and return complete, fixed files ready to deploy.
          </Text>
          <TouchableOpacity onPress={pickFiles} style={[styles.btn, { marginBottom: 16 }]} disabled={aiLoading}>
        ...
        <View>
          <Text style={{ color: '#888', fontSize: 12, marginBottom: 8 }}>📁 {uploadedFiles.length} files loaded</Text>
          ...
          {aiLoading && <Text style={{ color: '#00e5a0', fontSize: 13, padding: 8 }}>⚡ Moltbot working...</Text>}
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
            <TextInput value={chatInput2} onChangeText={setChatInput2} placeholder="Describe what to fix..." placeholderTextColor="#555" style={[styles.input, { flex: 1 }]} onSubmitEditing={sendAiChat} />
            <TouchableOpacity onPress={sendAiChat} disabled={aiLoading || !chatInput2.trim()} style={[styles.btn, { paddingHorizontal: 16 }]}>
              <Text style={styles.btnText}>Send</Text>
            </TouchableOpacity>
          </View>

          <TouchableOpacity onPress={() => { setUploadedFiles([]); setChatMsgs([]); setGenFiles([]); }} style={{ marginTop: 12, alignSelf: 'center' }}>
