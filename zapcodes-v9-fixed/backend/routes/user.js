...
router.get('/ai-preference', auth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      effectiveAI,
      plan: user.plan,
      canUseClaude: user.plan === 'starter' || user.plan === 'pro',
    });
  } catch (err) {
...
router.put('/ai-preference', auth, async (req, res) => {
  try {
    ...
    if (!['ollama', 'claude'].includes(preferredAI)) {
      return res.status(400).json({ error: 'Invalid AI option. Choose "ollama" or "claude".' });
    }
    ...
    res.json({
      ...
      message: `AI engine set to ${preferredAI === 'claude' ? 'Claude Opus 4.6' : 'Ollama (Free)'}`,
    });
  } catch (err) {
...
router.post('/chats/:projectId/message', auth, async (req, res) => {
  try {
    ...
    const limits = { free: 3, starter: 25, pro: 99999 };
