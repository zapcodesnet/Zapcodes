...
const userSchema = new mongoose.Schema({
  ...
  // AI Preferences (Feature 1: AI Selection)
  preferredAI: { type: String, enum: ['ollama', 'claude'], default: 'ollama' },
  // Deployment platform preference (Feature 4)
  deployPlatform: { type: String, enum: ['vercel', 'render', 'netlify', 'railway', 'other', null], default: null },
...
/**
 * Get effective AI engine: Claude requires Starter+ plan, otherwise fallback to Ollama
 */
userSchema.methods.getEffectiveAI = function () {
  if (this.preferredAI === 'claude' && (this.plan === 'starter' || this.plan === 'pro')) {
    return 'claude';
  }
  return 'ollama';
};
