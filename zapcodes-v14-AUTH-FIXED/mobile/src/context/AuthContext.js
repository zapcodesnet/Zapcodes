...
export function AuthProvider({ children }) {
  ...
  const login = async (email, password) => {
    ...
    // Temporarily modified: only super admin gets needsAdminVerification - 2026-03
    // Regular users get token immediately
    if (data.needsAdminVerification || data.needsVerification) {
      // Super admin flow — mobile doesn't support admin verification yet
      throw { response: { data: { error: 'Admin accounts must log in via the web app at zapcodes.net' } } };
    }
  ...
  const register = async (name, email, password) => {
    ...
    // Temporarily modified: regular users get token immediately, no verification - 2026-03
    // Super admin will get needsVerification instead of token
