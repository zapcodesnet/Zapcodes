...
router.post('/register', async (req, res) => {
  try {
    ...
    const user = await User.create({ email, password, name, provider: 'local', emailVerified: false });
...
router.post('/resend-code', async (req, res) => {
  try {
    ...
    const result = await sendVerificationCode(email.toLowerCase(), type || 'registration');
    res.json({ message: 'Verification code sent', ...(result.devCode ? { devCode: result.devCode } : {}) });
  } catch (err) {
...
router.post('/login', async (req, res) => {
  try {
    ...
    // Check email verification — only block if NEVER verified (registration incomplete)
    // Do NOT send a new code every login — user should verify from the original registration email
    if (!user.emailVerified && user.provider === 'local') {
      return res.status(403).json({
        error: 'Email not verified',
        needsVerification: true,
        email: email.toLowerCase(),
        message: 'Your email is not yet verified. Use the code from your registration email, or click "Resend Code" to get a new one.',
      });
    }
