...
export default function Settings() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-primary)' }}>
      <Sidebar />
      <main className="page-content" style={st.main}>
...
function GitHubSection() {
  ...
  useEffect(() => {
    api.get('/api/user/github-token/status').then(r => setStatus(r.data)).catch(() => {});
  }, []);
  ...
  const saveToken = async () => {
    ...
    try {
      await api.put('/api/user/github-token', { token: token.trim(), keepPermanent });
  ...
  const deleteToken = async () => {
    setLoading(true);
    try {
      await api.delete('/api/user/github-token');
  ...
  const testConnection = async () => {
    ...
    try {
      const { data } = await api.post('/api/user/github-token/test');
      setTestResult(data);
    ...
    <div style={st.card}>
      ...
      <div style={st.infoBox}>
        <p style={{ fontSize: '0.85rem', lineHeight: 1.7, color: 'var(--text-secondary)' }}>
          Connect your GitHub PAT so <strong>ZapCodes AI can auto-apply fixes</strong> to your repos, enable version control, auto-commits, and one-click deployments.
        </p>
        ...
        <div style={{ marginTop: 20 }}>
          ...
          {showDeleteConfirm && (
            <div style={{ marginTop: 12, background: 'rgba(255,68,102,0.05)', border: '1px solid rgba(255,68,102,0.2)', borderRadius: 10, padding: 16 }}>
              <p style={{ fontSize: '0.85rem', color: '#ff4466' }}>Remove GitHub token? ZapCodes AI won't be able to push fixes until re-added.</p>
...
function ProfileSection({ user }) {
  ...
  const save = async () => {
    setLoading(true);
    try { await api.put('/api/user/profile', { name }); setMsg('Profile updated!'); }
...
function PlanSection({ user }) {
  return (
    <div style={st.card}>
      ...
      <div style={st.row}><span style={st.rowLabel}>Plan:</span> <strong style={{ textTransform: 'capitalize', color: user?.plan === 'diamond' ? '#06b6d4' : user?.plan === 'gold' ? '#f59e0b' : user?.plan === 'silver' ? '#6366f1' : user?.plan === 'bronze' ? '#cd7f32' : 'var(--text-secondary)' }}>{user?.plan || 'Free'}</strong></div>
...
const st = {
  main: {
    maxWidth: 800, boxSizing: 'border-box',
    overflowY: 'auto', WebkitOverflowScrolling: 'touch',
  },
  title: { fontSize: '1.6rem', fontWeight: 800, marginBottom: 24 },
  card: { background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '24px 20px', marginBottom: 20 },
