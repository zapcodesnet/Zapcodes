...
export function AuthProvider({ children }) {
  ...
  const login = async (email, password) => {
    ...
    // If verification is needed, pass data back (no token/user stored yet)
    if (data.needsVerification || data.needsAdminVerification) {
      return data;
    }
  ...
  const completeVerifiedLogin = (data) => {
  ...
  const fetchRepos = useCallback(async () => {
    try {
      const { data } = await api.get('/api/scan/repos');
      setRepos(data.repos);
    } catch (e) {
  ...
  const scanRepo = async (url, engine = 'groq') => {
    setScanStatus({ status: 'scanning', message: 'Starting scan...' });
    try {
      const { data } = await api.post('/api/scan', { url, engine });
  ...
  const applyFix = async (repoId, issueId) => {
    const { data } = await api.post('/api/fix', { repoId, issueId });
  ...
  const dismissIssue = async (repoId, issueId) => {
    await api.post('/api/fix/dismiss', { repoId, issueId });
    if (currentRepo && currentRepo._id === repoId) {
      const updated = { ...currentRepo };
      const issue = updated.issues.find(i => i.id === issueId);
  ...
  const askTutorial = async (question) => {
    const { data } = await api.post('/api/tutorial', { question });
  ...
  return (
    <AuthContext.Provider value={{
      ...
      login, register, logout, handleAuthCallback, completeVerifiedLogin,
