...
const { callClaude, callClaudeWithImages, callAI, parseFilesFromResponse, CLAUDE_LARGE_OUTPUT, GROQ_MAX_OUTPUT } = require('../services/ai');
...
router.post('/analyze-with-images', optionalAuth, async (req, res) => {
  try {
    ...
    if (!aiResponse) {
      ...
      aiResponse = await callAI(systemPrompt, userPrompt + imageDesc);
    }
    if (!aiResponse) {
...
router.post('/analyze', optionalAuth, async (req, res) => {
  try {
    ...
    if (!aiResponse) {
      aiResponse = await callAI(systemPrompt, userPrompt, { maxTokens: GROQ_MAX_OUTPUT });
    }
...
router.post('/generate', optionalAuth, async (req, res) => {
  try {
    ...
    if (!aiResponse) {
      aiResponse = await callAI(GENERATE_PROMPT, userPrompt, { maxTokens: GROQ_MAX_OUTPUT });
    }
    ...
    for (const file of files) {
      try {
        ...
        await axios.put(`https://api.github.com/repos/${owner}/${repo.replace(/\.git$/,'')}/contents/${file.name}`, {
          message: commitMessage || `Update ${file.name} via ZapCodes AI`,
...
router.get('/ai-status', async (req, res) => {
  ...
  res.json({
    groq: { available: !!groqKey, keySet: !!groqKey },
...
const FIX_PROMPT = `You are ZapCodes AI, an expert full-stack developer. The user uploaded their ENTIRE repository and wants you to fix or improve something.
