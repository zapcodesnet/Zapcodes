...
function formatCap(n) {
  ...
  if (n >= 999999999 || !isFinite(n)) return '∞';
  return n.toLocaleString();
}
...
export default function Dashboard() {
  ...
  const genPct = tc.dailyGenCap ? ((du.generations || 0) / (tc.dailyGenCap >= 999999999 || !isFinite(tc.dailyGenCap) ? 100 : tc.dailyGenCap)) * 100 : 0;
  const fixPct = tc.dailyFixCap ? ((du.codeFixes || 0) / (tc.dailyFixCap >= 999999999 || !isFinite(tc.dailyFixCap) ? 100 : tc.dailyFixCap)) * 100 : 0;
      ...
      <div style={s.grid}>
        ...
        <div style={s.card}>
          ...
          <div style={s.limitsBox}>
            <div style={s.limitRow}><span>Max chars</span><span>{(tc.maxChars >= 999999999 || !isFinite(tc.maxChars)) ? '∞' : tc.maxChars?.toLocaleString()}</span></div>
