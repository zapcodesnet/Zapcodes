            ...
            <div style={{ maxWidth: 600, margin: '0 auto' }}>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
                ...
                <button onClick={handleGenerate} className="btn btn-primary" style={{ flex: 2 }} disabled={loading}>
                  {loading ? (aiEngine === 'claude' ? '🧠 Claude Opus 4.6 is generating... (30-60s)' : '🤖 AI is generating... (15-30s)') : '⚡ Generate My Project'}
                </button>
              </div>
            </div>
          )}
            ...
            <div>
              <div style={{ textAlign: 'center', marginBottom: 40 }}>
                ...
                <p style={{ color: 'var(--text-secondary)' }}>
                  {result.totalFiles} files generated
                  {result.generatedBy && result.generatedBy !== 'static-template' && (
                    <span style={{ marginLeft: 8, padding: '2px 10px', borderRadius: 100, fontSize: '0.75rem', fontWeight: 600, background: result.generatedBy.includes('claude') ? 'rgba(163, 85, 247, 0.15)' : 'rgba(0, 229, 160, 0.1)', color: result.generatedBy.includes('claude') ? '#c084fc' : '#00e5a0' }}>
                      {result.generatedBy.includes('claude') ? '🧠 AI: Claude Opus 4.6' : '🤖 AI: Groq'}
                    </span>
                  )}
                  {result.generatedBy === 'static-template' && (
                    <span style={{ marginLeft: 8, padding: '2px 10px', borderRadius: 100, fontSize: '0.75rem', fontWeight: 600, background: 'rgba(251, 146, 60, 0.1)', color: '#fb923c' }}>
                      📦 Static Template
                    </span>
                  )}
                </p>
                {result.sizeWarning && (
                  <p style={{ marginTop: 8, padding: '8px 16px', background: 'rgba(251, 146, 60, 0.08)', border: '1px solid rgba(251, 146, 60, 0.2)', borderRadius: 8, fontSize: '0.8rem', color: '#fb923c' }}>
                    ⚠️ {result.sizeWarning}
                  </p>
                )}
              </div>
...
// =====================================================================
// AI ENGINE SELECTOR (Feature 1)
// =====================================================================
function AISelector({ engine, onEngineChange }) {
  const { user } = useAuth();
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>AI:</span>
      <select
        value={engine}
        onChange={e => {
          const val = e.target.value;
          if (val === 'claude' && !canUseClaude) {
            alert('Claude Opus 4.6 requires a Starter ($9/mo) or Pro ($29/mo) plan. Free users get Ollama.');
            return;
          }
          onEngineChange(val);
          // Persist preference to backend
          if (user) {
            api.put('/user/ai-preference', { preferredAI: val }).catch(() => {});
          }
        }}
        style={{
          background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 6,
          padding: '4px 8px', color: 'var(--text-primary)', fontSize: '0.72rem', cursor: 'pointer',
          appearance: 'auto',
        }}
      >
        <option value="ollama">🟢 Ollama (Free)</option>
        <option value="claude" disabled={!canUseClaude}>
          {canUseClaude ? '🟣 Claude Opus 4.6' : '🔒 Claude Opus 4.6 (Starter+)'}
        </option>
      </select>
      {engine === 'claude' && (
        <span style={{ fontSize: '0.65rem', color: '#a855f7', background: 'rgba(168,85,247,0.1)', padding: '2px 6px', borderRadius: 4 }}>Pro AI</span>
      )}
    </div>
  );
}
...
// =====================================================================
// FILE UPLOAD + AI CHAT (with all 5 features integrated)
// =====================================================================
function FileUploadChat() {
  ...
  const [aiEngine, setAiEngine] = useState(user?.preferredAI || 'ollama');

  // Feature 5: Image upload state
  const [uploadedImages, setUploadedImages] = useState([]);
  const [imageDragOver, setImageDragOver] = useState(false);

  // Feature 4: Show deployment instructions
  ...
  // Load AI preference from backend on mount
  useEffect(() => {
    if (user) {
  ...
  // Feature 5: Handle image upload
  const handleImageUpload = async (fileList) => {
    const imageFiles = Array.from(fileList).filter(f => f.type.startsWith('image/'));
    if (!imageFiles.length) { alert('Please upload image files (PNG, JPG, GIF, WebP)'); return; }

    const formData = new FormData();
    for (const f of imageFiles) formData.append('images', f);

    try {
      const { data } = await api.post('/files/upload-images', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  const sendChat = async () => {
    ...
    // Feature 5: If images are attached, require description
    if (uploadedImages.length > 0 && msg.length < 10) {
      alert('Please describe the issue shown in the image(s) (at least 10 characters).');
      return;
    }
    ...
    setChatMessages(prev => [...prev, {
      role: 'user',
      content: msg + (uploadedImages.length ? ` [📷 ${uploadedImages.length} image(s) attached]` : ''),
    }]);
    setLoading(true);
    setGeneratedFiles([]);
    ...
    try {
      ...
      if (uploadedImages.length > 0) {
        ...
        const response = await api.post('/files/analyze', { files, prompt: msg, mode, engine: aiEngine });
        data = response.data;
  ...
  const generateFiles = async () => {
    ...
    try {
      ...
      const { data } = await api.post('/files/generate', { files, prompt: prompts, engine: aiEngine });
      if (data.generatedFiles?.length > 0) {
        setGeneratedFiles(data.generatedFiles);
        setShowDeployGuide(true);
        setChatMessages(prev => [...prev, {
  ...
  // Feature 3: Download all files as proper ZIP
  const downloadAllZip = async () => {
    setChatMessages(prev => [...prev, { role: 'system', content: '⚡ Generating ZIP file...' }]);
    try {
      const { data } = await api.post('/files/generate-zip', { files: generatedFiles });
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = data.filename || 'zapcodes-project.zip';
      a.click();
      URL.revokeObjectURL(url);
      setChatMessages(prev => [...prev, {
        role: 'assistant',
        content: `📦 **ZIP downloaded!** (${data.fileCount} files, ${(data.size / 1024).toFixed(1)} KB)\n\nReplace files in your repo at their exact paths, then \`git push\` to deploy.`,
      }]);
    } catch (err) {
      // Fallback: download individually
      for (const f of generatedFiles) downloadFile(f);
      setChatMessages(prev => [...prev, { role: 'assistant', content: `⬇️ **${generatedFiles.length} files downloaded individually.** Replace them at their exact paths in your repo, then \`git push\`.` }]);
    }
  };
  ...
  const pushToGitHub = async () => {
    ...
    try {
      const filesToPush = generatedFiles.length > 0 ? generatedFiles : (await api.post('/files/generate', { files, prompt: chatMessages.filter(m => m.role === 'user').map(m => m.content).join('; '), engine: aiEngine })).data.generatedFiles;
  ...
  // Feature 2: Line numbers for textarea
  const lineCount = chatInput.split('\n').length;
  ...
  return (
    <div style={fcS.card}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        {/* Feature 1: AI selector in header */}
        {showChat && <AISelector engine={aiEngine} onEngineChange={setAiEngine} />}
      </div>

      {!showChat ? (
        <>
          <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginBottom: 14, lineHeight: 1.6 }}>
            Upload your entire repo (ZIP) or individual files. When you ask for a fix, the AI returns <strong>complete, ready-to-deploy files</strong> — not snippets. Copy-paste directly into your repo.
          </p>
        ...
        <>
          ...
          <div ref={chatRef} style={{ maxHeight: 300, overflowY: 'auto', marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {loading && <div style={{ alignSelf: 'flex-start', fontSize: '0.8rem', color: 'var(--text-muted)', padding: '8px 0' }}>⚡ {aiEngine === 'claude' ? 'Claude Opus 4.6' : 'Ollama'} is working... (may take 15-30s)</div>}
          </div>
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, flexWrap: 'wrap', gap: 6 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {/* Feature 3: ZIP download button */}
                  <button onClick={downloadAllZip} style={{ ...fcS.btnAccent, background: '#6366f1', color: '#fff' }}>📦 Download ZIP</button>
          ...
          {/* Feature 4: Deployment Instructions */}
          {showDeployGuide && generatedFiles.length > 0 && (
            <DeploymentGuide onDismiss={() => setShowDeployGuide(false)} />
          )}

          {/* Feature 5: Image upload area */}
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
              {uploadedImages.map((img, i) => (
                <div key={i} style={{ position: 'relative', width: 60, height: 60, borderRadius: 8, overflow: 'hidden', border: '1px solid var(--border)' }}>
                  <img src={`data:${img.mimeType};base64,${img.base64}`} alt={img.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  <button onClick={() => removeImage(i)} style={{ position: 'absolute', top: -2, right: -2, width: 18, height: 18, borderRadius: '50%', background: '#ff4466', color: '#fff', border: 'none', fontSize: '0.6rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
                </div>
              ))}
            </div>
          )}

          {/* Feature 2: Enlarged prompt input with line numbers */}
          <div style={{ position: 'relative', marginBottom: 8 }}>
            {/* Line numbers gutter */}
            {chatInput.includes('\n') && (
              <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 32, background: 'rgba(0,0,0,0.2)', borderRadius: '8px 0 0 8px', pointerEvents: 'none', zIndex: 1, display: 'flex', flexDirection: 'column', paddingTop: 10, alignItems: 'center' }}>
                {Array.from({ length: lineCount }, (_, i) => (
                  <div key={i} style={{ fontSize: '0.65rem', color: 'var(--text-muted)', lineHeight: '1.45rem', opacity: 0.5 }}>{i + 1}</div>
                ))}
              </div>
            )}
            <textarea
              ...
              placeholder={uploadedImages.length > 0
                ? "Describe the issue shown in the image(s)... (required)"
                : "Describe what to fix — I'll return complete files..."}
              style={{
                width: '100%',
                minHeight: 80,
                maxHeight: 300,
                ...
                borderRadius: 8,
                padding: chatInput.includes('\n') ? '10px 14px 10px 38px' : '10px 14px',
                color: 'var(--text-primary)',
                fontSize: '0.85rem',
                fontFamily: chatInput.includes('{') || chatInput.includes('function') ? 'monospace' : 'inherit',
                lineHeight: '1.45rem',
                resize: 'vertical',
            ...
            {/* Preview toggle */}
            {chatInput.length > 50 && (
              <button onClick={() => setShowPreview(!showPreview)} style={{ position: 'absolute', right: 8, top: 8, ...fcS.btn, fontSize: '0.65rem' }}>
          ...
          {/* Preview panel */}
          {showPreview && chatInput && (
            <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 8, padding: 12, marginBottom: 8, fontSize: '0.82rem', lineHeight: 1.6 }}
              dangerouslySetInnerHTML={{ __html: fmtMsg(chatInput) }} />
          )}

          {/* Action buttons row */}
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            <button onClick={sendChat} disabled={loading || !chatInput.trim()} style={{ padding: '10px 16px', borderRadius: 8, border: 'none', background: 'var(--accent)', color: '#06060b', fontWeight: 700, cursor: 'pointer' }}>
            ...
            <button
              ...
              style={{ ...fcS.btn, display: 'flex', alignItems: 'center', gap: 4 }}
              title="Upload screenshot(s) of the issue"
            >
              📷 Image{uploadedImages.length > 0 ? ` (${uploadedImages.length})` : ''}
            </button>
            <input ref={imageRef} type="file" multiple accept="image/*" onChange={e => handleImageUpload(e.target.files)} style={{ display: 'none' }} />

            {/* Image drag-drop area (hidden unless dragging) */}
            <div
              onDragOver={e => { e.preventDefault(); if (e.dataTransfer.types.includes('Files')) setImageDragOver(true); }}
              onDragLeave={() => setImageDragOver(false)}
              onDrop={e => { e.preventDefault(); setImageDragOver(false); handleImageUpload(e.dataTransfer.files); }}
              style={{ display: imageDragOver ? 'flex' : 'none', position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,229,160,0.08)', zIndex: 999, alignItems: 'center', justifyContent: 'center', border: '3px dashed #00e5a0' }}
            >
              <div style={{ background: 'var(--bg-card)', padding: 40, borderRadius: 16, textAlign: 'center' }}>
                <p style={{ fontSize: '2rem' }}>📷</p>
                <p style={{ color: 'var(--text-primary)', fontSize: '1.1rem', fontWeight: 700 }}>Drop image(s) here</p>
              </div>
            </div>
            ...
            <button onClick={() => { setShowChat(false); setFiles([]); setChatMessages([]); setGeneratedFiles([]); setUploadedImages([]); }} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: '0.8rem', cursor: 'pointer' }}>← Upload new files</button>
          </div>
          ...
          {showPushModal && (
            <div style={{ marginTop: 12, background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 10, padding: 16 }}>
              ...
              <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: 10 }}>Moltbot only applies fixes — it does not analyze or suggest changes. Files will be committed directly to your repo. Make sure your GitHub token is connected in Settings.</p>
...
// =====================================================================
// DEPLOYMENT GUIDE (Feature 4)
// =====================================================================
function DeploymentGuide({ onDismiss }) {
  const [expanded, setExpanded] = useState(false);
  const [platform, setPlatform] = useState('both');

  return (
    <div style={{ background: 'rgba(99,102,241,0.04)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 12, padding: 16, marginBottom: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: expanded ? 12 : 0 }}>
        <button onClick={() => setExpanded(!expanded)} style={{ background: 'none', border: 'none', color: 'var(--text-primary)', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8 }}>
          🚀 Deployment Instructions {expanded ? '▲' : '▼'}
        </button>
        <button onClick={onDismiss} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '0.75rem' }}>✕</button>
      </div>
      ...
      {expanded && (
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            {['both', 'vercel', 'render'].map(p => (
              <button key={p} onClick={() => setPlatform(p)} style={{
                padding: '6px 14px', borderRadius: 8, border: '1px solid var(--border)',
                background: platform === p ? 'var(--accent)' : 'var(--bg-elevated)',
                color: platform === p ? '#06060b' : 'var(--text-muted)',
                fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer',
              }}>
                {p === 'both' ? '🌐 Both' : p === 'vercel' ? '▲ Vercel' : '🟣 Render'}
              </button>
            ))}
          </div>

          {/* Vercel instructions */}
          {(platform === 'both' || platform === 'vercel') && (
            <div style={{ marginBottom: 16 }}>
              <h4 style={{ fontSize: '0.9rem', fontWeight: 700, marginBottom: 10, color: 'var(--text-primary)' }}>▲ Frontend → Vercel</h4>
              {[
                { n: 1, t: 'Log in', d: 'Go to vercel.com and sign in' },
                { n: 2, t: 'Select Project', d: 'Click your project (or "Import Project" if new)' },
                { n: 3, t: 'Settings', d: 'Go to Settings → General' },
                { n: 4, t: 'Root Directory', d: 'Set to "web" (where your frontend code lives)' },
                { n: 5, t: 'Build Command', d: 'npm run build' },
                { n: 6, t: 'Output Directory', d: 'dist' },
                { n: 7, t: 'Environment Variables', d: 'Settings → Environment Variables → Add: VITE_API_URL = https://your-api.onrender.com/api' },
                { n: 8, t: 'Deploy', d: 'Click "Redeploy" or push to git — auto-deploys in ~60 seconds' },
              ].map(s => (
                <div key={s.n} style={{ display: 'flex', gap: 10, marginBottom: 8 }}>
                  <span style={{ background: 'var(--accent)', color: '#06060b', width: 22, height: 22, minWidth: 22, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', fontWeight: 800 }}>{s.n}</span>
                  <div>
                    <strong style={{ fontSize: '0.82rem' }}>{s.t}</strong>
                    <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: 2 }}>{s.d}</p>
                  </div>
                </div>
              ))}
              <div style={{ background: 'rgba(0,229,160,0.06)', borderRadius: 8, padding: 10, marginTop: 8, fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                💡 Clear browser cache after deploy · Check build logs at vercel.com/dashboard
              </div>
            </div>
          )}

          {/* Render instructions */}
          {(platform === 'both' || platform === 'render') && (
            <div>
              <h4 style={{ fontSize: '0.9rem', fontWeight: 700, marginBottom: 10, color: 'var(--text-primary)' }}>🟣 Backend → Render</h4>
              {[
                { n: 1, t: 'Log in', d: 'Go to render.com and sign in' },
                { n: 2, t: 'Select Service', d: 'Click your Web Service (or create new from GitHub)' },
                { n: 3, t: 'Root Directory', d: 'Set to "backend"' },
                { n: 4, t: 'Build Command', d: 'npm install' },
                { n: 5, t: 'Start Command', d: 'node server.js' },
                { n: 6, t: 'Environment Variables', d: 'Add all required vars:' },
                { n: 7, t: 'Deploy', d: 'Click "Manual Deploy" → "Deploy latest commit" — takes ~2-3 min' },
              ].map(s => (
                <div key={s.n} style={{ display: 'flex', gap: 10, marginBottom: 8 }}>
                  <span style={{ background: '#6366f1', color: '#fff', width: 22, height: 22, minWidth: 22, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', fontWeight: 800 }}>{s.n}</span>
                  <div>
                    <strong style={{ fontSize: '0.82rem' }}>{s.t}</strong>
                    <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: 2 }}>{s.d}</p>
                  </div>
              ...
              {/* Env vars checklist */}
              <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 8, padding: 12, marginTop: 8 }}>
                <strong style={{ fontSize: '0.78rem', marginBottom: 8, display: 'block' }}>Required Environment Variables:</strong>
                {['MONGODB_URI', 'JWT_SECRET', 'GROQ_API_KEY', 'ANTHROPIC_API_KEY', 'WEB_URL', 'STRIPE_SECRET_KEY', 'RESEND_API_KEY'].map(v => (
                  <div key={v} style={{ fontSize: '0.75rem', padding: '3px 0', fontFamily: 'monospace', color: 'var(--text-secondary)' }}>
                    ☐ <span style={{ color: '#ffaa00' }}>{v}</span> = your_value_here
                  </div>
                ))}
              </div>
              <div style={{ background: 'rgba(99,102,241,0.06)', borderRadius: 8, padding: 10, marginTop: 8, fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                💡 Check Render logs if server crashes · Restart if env vars changed · Use /api/files/ai-status to verify AI is working
              </div>
            </div>
          )}

          {/* Warning */}
          <div style={{ background: 'rgba(255,170,0,0.06)', border: '1px solid rgba(255,170,0,0.15)', borderRadius: 8, padding: 10, marginTop: 12, fontSize: '0.75rem', color: '#ffaa00' }}>
            ⚠️ After deploying, always clear your browser cache and test all functionality. If auto-deploy doesn't trigger, manually redeploy from your hosting dashboard.
          </div>
        </div>
      )}
    </div>
      ...
      {expanded && (
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, padding: '12px 16px', borderTop: '1px solid var(--border)' }}>
            <textarea value={input} onChange={e => setInput(e.target.value)} placeholder="Improve features, fix bugs, add functionality..." style={{ flex: 1, background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 14px', color: 'var(--text-primary)', fontSize: '0.85rem', minHeight: 40, maxHeight: 120, resize: 'vertical', fontFamily: 'inherit' }} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(); } }} disabled={loading} />
            <button onClick={send} disabled={loading || !input.trim()} style={{ padding: '10px 20px', borderRadius: 8, border: 'none', background: 'var(--accent)', color: '#06060b', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', alignSelf: 'flex-end' }}>Send</button>
            <button onClick={forkChat} title="Fork to new chat" style={{ padding: '10px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '0.8rem', alignSelf: 'flex-end' }}>🔀</button>
          </div>
        </div>
      )}
    </div>
