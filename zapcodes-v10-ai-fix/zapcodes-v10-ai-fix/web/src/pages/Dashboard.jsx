...
function ScanFileSection() {
  ...
  const toggleFix = (id) => { const next = new Set(selectedFixes); next.has(id) ? next.delete(id) : next.add(id); setSelectedFixes(next); };
  ...
  const downloadFile = (file) => { const b = new Blob([file.content], { type: 'text/plain' }); const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = file.name.split('/').pop(); a.click(); };
  const copyFile = (idx) => { navigator.clipboard.writeText(generated.generatedFiles[idx].content); setCopiedIdx(idx); setTimeout(() => setCopiedIdx(-1), 2000); };
  const downloadAll = () => { for (const f of generated.generatedFiles) downloadFile(f); };
  ...
  const pushViaBot = async () => {
    ...
    try {
      ...
      setPushResult(data);
    } catch (err) { setPushResult({ error: err.response?.data?.error || 'Push failed — connect your GitHub token in Settings first.' }); }
    setPushing(false);
  };
  ...
  return (
    <div className="card" style={{ marginTop: 24 }}>
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: 16 }}>
        Upload your repo (ZIP) or files — Moltbot scans for bugs, then returns <strong>complete, ready-to-deploy files</strong> with fixes applied.
      </p>

      {/* Drop zone */}
      <div onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={e => { e.preventDefault(); setDragOver(false); handleFiles(e.dataTransfer.files); }} onClick={() => fileRef.current?.click()} style={{ border: `2px dashed ${dragOver ? '#00e5a0' : 'var(--border)'}`, borderRadius: 12, padding: 32, textAlign: 'center', cursor: 'pointer', background: dragOver ? 'rgba(0,229,160,0.03)' : 'transparent', transition: '0.2s' }}>
      ...
      {files.length > 0 && (
        <div style={{ marginTop: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={() => setShowTree(!showTree)} style={{ background: 'none', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 10px', fontSize: '0.72rem', color: 'var(--text-muted)', cursor: 'pointer' }}>{showTree ? 'Hide' : 'Show'} tree</button>
        </div>
      )}
      {showTree && (
        <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 8, padding: 10, marginTop: 8, maxHeight: 150, overflowY: 'auto', fontSize: '0.72rem', fontFamily: 'monospace', color: 'var(--text-muted)' }}>
          ...
          {selectedFixes.size > 0 && (
            <div style={{ marginTop: 16, padding: 16, background: 'var(--accent-glow)', border: '1px solid rgba(0,229,160,0.2)', borderRadius: 10 }}>
              ...
              <button onClick={generateFixed} disabled={generating} className="btn btn-primary" style={{ marginTop: 10, width: '100%' }}>
                {generating ? '⚡ Generating complete files...' : '⚡ Generate Complete Fixed Files'}
              </button>
            </div>
          )}
        </div>
      ...
      {/* Generated Files — with copy + download per file */}
      {generated?.generatedFiles?.length > 0 && (
        <div style={{ marginTop: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <h3 style={{ fontSize: '1rem', fontWeight: 700 }}>✅ {generated.generatedFiles.length} Complete File(s) Ready</h3>
            <div style={{ display: 'flex', gap: 6 }}>
              <button onClick={downloadAll} style={{ background: 'none', border: '1px solid var(--border)', borderRadius: 6, padding: '5px 10px', fontSize: '0.72rem', color: 'var(--text-muted)', cursor: 'pointer' }}>⬇️ Download All</button>
            </div>
          </div>
          {generated.summary && <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', marginBottom: 12, padding: 10, background: 'var(--bg-elevated)', borderRadius: 8, lineHeight: 1.6 }}>{generated.summary}</p>}

          {generated.generatedFiles.map((f, i) => (
            <div key={i} style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', marginBottom: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: 'rgba(0,229,160,0.04)', borderBottom: '1px solid var(--border)' }}>
                <span style={{ fontSize: '0.82rem', fontWeight: 600, fontFamily: 'monospace' }}>📄 {f.name}</span>
                <div style={{ display: 'flex', gap: 4 }}>
                  <button onClick={() => copyFile(i)} style={{ background: 'none', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 10px', fontSize: '0.72rem', color: 'var(--text-muted)', cursor: 'pointer' }}>{copiedIdx === i ? '✅ Copied!' : '📋 Copy'}</button>
                  <button onClick={() => downloadFile(f)} style={{ background: 'none', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 10px', fontSize: '0.72rem', color: 'var(--text-muted)', cursor: 'pointer' }}>⬇️</button>
                </div>
              </div>
              <pre style={{ padding: 12, background: 'var(--bg-elevated)', fontSize: '0.73rem', maxHeight: 250, overflow: 'auto', color: 'var(--text-secondary)', margin: 0, fontFamily: 'monospace', lineHeight: 1.5, whiteSpace: 'pre' }}>{f.content.length > 3000 ? f.content.slice(0, 3000) + '\n\n... (' + (f.content.length - 3000) + ' more chars — use Copy for full file)' : f.content}</pre>
            </div>
          ))}
          ...
          {generated.instructions && (
            <div style={{ marginTop: 16, padding: 16, background: 'var(--bg-elevated)', borderRadius: 10, border: '1px solid var(--border)' }}>
              <h4 style={{ fontSize: '0.9rem', fontWeight: 700, marginBottom: 10 }}>📋 How to apply:</h4>
              {generated.instructions.github.map((s, i) => <p key={i} style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', padding: '3px 0' }}>{i + 1}. {s}</p>)}
            </div>
          )}

          {/* Moltbot Push */}
          <div style={{ marginTop: 16, padding: 16, background: 'rgba(0,229,160,0.03)', borderRadius: 10, border: '1px solid rgba(0,229,160,0.15)' }}>
            <h4 style={{ fontSize: '0.9rem', fontWeight: 700, marginBottom: 8, color: 'var(--accent)' }}>🤖 Or: Push directly via Moltbot</h4>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              <button onClick={pushViaBot} disabled={pushing || !repoUrl.trim()} className="btn btn-primary" style={{ whiteSpace: 'nowrap' }}>{pushing ? '🤖 Pushing...' : '🤖 Push via Moltbot'}</button>
            </div>
