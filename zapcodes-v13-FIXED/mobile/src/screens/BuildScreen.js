...
export default function BuildScreen() {
  ...
  // Feature 1: AI engine selector — #6: Groq available for all tiers
  const [aiEngine, setAiEngine] = useState('groq');
  const plan = user?.plan || 'free';
  const canUseHaiku = ['silver', 'gold', 'diamond'].includes(plan);
  const canUseOpus = plan === 'diamond';

  // Load AI preference on mount
  useEffect(() => {
    if (user) {
      api.get('/user/ai-preference').then(({ data }) => {
        setAiEngine(data.effectiveAI || 'groq');
      }).catch(() => {});
    }
  }, [user]);
  ...
  const handleAIChange = (engine) => {
    if (engine === 'haiku' && !canUseHaiku) {
      Alert.alert('Upgrade Required', 'Claude Haiku requires Silver plan or higher.');
      return;
    }
    if (engine === 'opus' && !canUseOpus) {
      Alert.alert('Upgrade Required', 'Claude Opus 4.6 requires Diamond plan.');
      return;
    }
    setAiEngine(engine);
    if (user) {
  ...
  const renderUploadMode = () => (
    <View style={{ paddingHorizontal: 16 }}>
      {/* Feature 1: AI Selector — #6: Groq for all, Haiku for Silver+, Opus for Diamond */}
      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16, backgroundColor: '#11111b', borderRadius: 10, padding: 12, borderWidth: 1, borderColor: '#2a2a3a' }}>
        <Text style={{ color: '#888', fontSize: 13, marginRight: 10 }}>AI:</Text>
        <TouchableOpacity
          onPress={() => handleAIChange('groq')}
          style={{ flex: 1, padding: 8, borderRadius: 8, alignItems: 'center', backgroundColor: aiEngine === 'groq' ? '#00e5a0' : 'transparent' }}
        >
          <Text style={{ color: aiEngine === 'groq' ? '#06060b' : '#888', fontWeight: '700', fontSize: 12 }}>🟢 Groq</Text>
          <Text style={{ color: aiEngine === 'groq' ? '#06060b' : '#555', fontSize: 10 }}>5K BL</Text>
        </TouchableOpacity>
        {canUseHaiku && (
          <TouchableOpacity
            onPress={() => handleAIChange('haiku')}
            style={{ flex: 1, padding: 8, borderRadius: 8, alignItems: 'center', backgroundColor: aiEngine === 'haiku' ? '#6366f1' : 'transparent' }}
          >
            <Text style={{ color: aiEngine === 'haiku' ? '#fff' : '#888', fontWeight: '700', fontSize: 12 }}>🟣 Haiku</Text>
            <Text style={{ color: aiEngine === 'haiku' ? '#ddd' : '#555', fontSize: 10 }}>10K BL</Text>
          </TouchableOpacity>
        )}
        {canUseOpus && (
          <TouchableOpacity
            onPress={() => handleAIChange('opus')}
            style={{ flex: 1, padding: 8, borderRadius: 8, alignItems: 'center', backgroundColor: aiEngine === 'opus' ? '#a855f7' : 'transparent' }}
          >
            <Text style={{ color: aiEngine === 'opus' ? '#fff' : '#888', fontWeight: '700', fontSize: 12 }}>🔮 Opus</Text>
            <Text style={{ color: aiEngine === 'opus' ? '#ddd' : '#555', fontSize: 10 }}>50K BL</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      {uploadedFiles.length === 0 ? (
        ...
        <View>
          <Text style={{ color: '#888', fontSize: 12, marginBottom: 8 }}>📁 {uploadedFiles.length} files loaded · AI: {aiEngine === 'claude' ? '🟣 Claude' : '🟢 Groq'}</Text>
          ...
          {aiLoading && <Text style={{ color: '#00e5a0', fontSize: 13, padding: 8 }}>⚡ {aiEngine === 'claude' ? 'Claude Opus 4.6' : 'Groq'} working...</Text>}
