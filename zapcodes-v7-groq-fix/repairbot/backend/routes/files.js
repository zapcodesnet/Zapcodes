...
router.post('/analyze', optionalAuth, async (req, res) => {
  try {
    ...
    if (!GROQ_KEY) {
      return res.json({
        analysis: generateFallbackAnalysis(files, mode),
        issues: mode === 'scan' ? tryParseFallback(generateFallbackAnalysis(files, mode)) : undefined,
        tokenEstimate,
        mode,
        warning: 'GROQ_API_KEY not set — using basic static analysis',
      });
    }

    // Try multiple models in order (Groq deprecates models frequently)
    const models = [
      ...
      try {
        ...
        const groqRes = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
          ...
          messages: [
            ...
            { role: 'user', content: userPrompt.slice(0, 28000) }, // Safe limit for most models
          ],
          temperature: 0.3,
          max_tokens: 4096,
        }, {
        ...
        console.error(`[GROQ] Model ${model} failed (${status}): ${detail}`);
        lastError = { model, status, detail };

        // If 401 = bad API key, don't try more models
        if (status === 401) break;
        // If 429 = rate limited, don't try more models
        if (status === 429) break;
      }
    }

    if (!aiResponse) {
      console.error('[GROQ] All models failed, using fallback. Last error:', lastError);
      aiResponse = generateFallbackAnalysis(files, mode);
      // Include error info so user knows what happened
      if (lastError?.status === 401) {
        return res.json({ analysis: 'GROQ API key is invalid. Please check the RESEND_API_KEY in Render environment variables.', tokenEstimate, mode, error: 'Invalid GROQ_API_KEY' });
      }
    }
...
router.post('/generate', optionalAuth, async (req, res) => {
  try {
    ...
    if (!GROQ_KEY) return res.status(502).json({ error: 'GROQ_API_KEY not configured' });
    ...
    for (const model of models) {
      try {
        ...
        const status = err.response?.status;
        console.error(`[GROQ generate] ${model} failed (${status}): ${err.response?.data?.error?.message || err.message}`);
        if (status === 401 || status === 429) break;
      }
    }

    if (!aiResponse) return res.status(502).json({ error: 'AI generation failed — all models unavailable. Check GROQ_API_KEY.' });
...
function tryParseFallback(text) {
  try {
...
// GET /api/files/groq-status — Check if Groq API is working
router.get('/groq-status', async (req, res) => {
  const key = process.env.GROQ_API_KEY;
  if (!key) return res.json({ status: 'error', message: 'GROQ_API_KEY not set in environment variables' });
  ...
  for (const model of models) {
    try {
      ...
      results.push({ model, status: 'ok', ms: Date.now() - start });
    } catch (err) {
      results.push({ model, status: 'fail', error: err.response?.data?.error?.message || err.message, code: err.response?.status });
    }
  }

  const working = results.filter(r => r.status === 'ok');
  res.json({
    status: working.length > 0 ? 'ok' : 'error',
