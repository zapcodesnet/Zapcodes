...
function FileUploadChat() {
  ...
  const sendChat = async () => {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { role: 'system', content: '❌ Analysis failed. Please try again.' }]);
    }
    setLoading(false);
  };
  ...
  const pushToGitHub = async () => {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { role: 'assistant', content: `🤖 **Moltbot pushed to GitHub!** ${pushData.message}\n\n${pushData.results.map(r => `${r.status === 'success' ? '✅' : '❌'} ${r.name}: ${r.action || r.error}`).join('\n')}\n\n🚀 **Deploy:** If using Vercel/Render with auto-deploy, your changes are live in ~60s. Otherwise push to trigger a build.` }]);
      setShowPushModal(false);
    } catch (err) {
      setPushResult({ error: err.response?.data?.error || 'Push failed — is your GitHub token connected in Settings?' });
    }
    setLoading(false);
  };
      ...
      {!showChat ? (
        <>
          <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 14, lineHeight: 1.6 }}>
            Upload your code files (ZIP or individual) — Moltbot analyzes, suggests improvements, and generates fixed versions.
          </p>
          <div
        ...
        <>
          ...
          <div ref={chatRef} style={{ maxHeight: 350, overflowY: 'auto', marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {loading && <div style={{ alignSelf: 'flex-start', fontSize: '0.8rem', color: 'var(--text-muted)' }}>⚡ Processing...</div>}
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
            <button onClick={generateAndDownload} disabled={loading} style={{ padding: '8px 14px', borderRadius: 8, border: '1px solid var(--border)', background: 'none', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: '0.8rem' }}>⬇️ Download Fixed Files</button>
            <button onClick={() => setShowPushModal(true)} disabled={loading} style={{ padding: '8px 14px', borderRadius: 8, border: '1px solid rgba(0,229,160,0.3)', background: 'rgba(0,229,160,0.08)', color: '#00e5a0', cursor: 'pointer', fontSize: '0.8rem', fontWeight: 600 }}>🤖 Apply via Moltbot</button>
            <button onClick={() => { setShowChat(false); setFiles([]); setChatMessages([]); }} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: '0.8rem', cursor: 'pointer' }}>← New files</button>
          </div>

          {/* Moltbot Push Modal */}
          {showPushModal && (
            <div style={{ marginTop: 12, background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 10, padding: 16 }}>
              <p style={{ fontSize: '0.85rem', fontWeight: 600, marginBottom: 8 }}>🤖 Push to GitHub via Moltbot</p>
