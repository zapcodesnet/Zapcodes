...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const savedUser = await AsyncStorage.getItem('repairbot_user');
        if (token && savedUser) {
  ...
  const login = async (email, password) => {
    ...
    await AsyncStorage.setItem('repairbot_token', data.token);
    await AsyncStorage.setItem('repairbot_user', JSON.stringify(data.user));
    setUser(data.user);
