...
export default function ProfileScreen() {
  ...
  const saveGithubToken = async () => {
    ...
    try {
      ...
      Alert.alert('Saved', 'GitHub token saved. You can now apply fixes via Moltbot.');
      setGithubToken('');
    } catch (err) {
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.helpText}>
          Required to apply fixes via Moltbot. Generate a token at github.com/settings/tokens with repo scope.
        </Text>
      ...
      {user?.plan === 'free' && (
        <View style={[styles.card, { borderColor: colors.accent + '44' }]}>
          ...
          <Text style={styles.helpText}>
            Get unlimited scans, Claude Pro AI engine, priority support, and auto-fix via Moltbot.
          </Text>
      ...
      <Text style={styles.version}>RepairBot v1.0.0</Text>
